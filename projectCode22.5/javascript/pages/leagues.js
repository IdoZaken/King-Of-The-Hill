let leagues = function($container){
	...
	//buttons listeners
	$("#cmdFilterByLeague").click(leaguesClickListener).click();	//click when we open this page to show leagues
	$("#cmdFilterByDate").click(leaguesClickListener);
	
	//$("#cmdFilterByLeague").click();
...
function addLeagueToList(doc){
	...
	$("ul").append(
		"<li><button onclick = 'goToGames(\""+leagueName+"\")'>"+doc.get('Name')+"	in "+doc.get('State')+"</button></li>"
	);
}

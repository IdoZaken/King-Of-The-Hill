let home = function($container){
	...
	$container.html(
		...
		"<div id = 'navBar' class = 'navBar' style = 'visibility : hidden'>"+//Create a hidden navigation bar
				...
				"<button id = 'cmdNavAdmin' class = 'navBar admin'><!-- ADMIN --></button>"+
		"</div>"+
		"<div id = 'home'>"+
			"<main>"+
				"<p><section>"+
			...
			"</nav>"+
			"<footer>"+
				"<button id = 'cmdAdmin' class = 'admin' style = 'display: none;'><!-- ADMIN --></button>"+
	...
	//TODO: move rt listener that change Locked state to server function and not on home screen
	// because if you on diffrent screen and game need to be locked' it want be locked

	//listener to games in DB to update for current time if game is open or locked
	firebase.firestore().collection("leagues/Top League/games").where("Locked", "==", false)
    .onSnapshot(function(querySnapshot) {
		var now = new Date();
		now = now.getTime()/1000;
        querySnapshot.forEach(function(doc) {
			if(now >= doc.get("Time").seconds){
				docRef = db.doc("leagues/Top League/games/"+doc.id);
				docRef.update({
					Locked: true
				})
				.then(function() {
	...
	// Send token to your backend via HTTPS
		if(idTokenResult.claims.admin){
			$("#cmdAdmin").css("display", "inline-block");
		}
		else{
...
let navBarClickListener = function(e){
	let targetId = e.target.id;		//e.target.id shortcut
	$(".navBar").css("display", "inline");	//display all buttons in nav bar

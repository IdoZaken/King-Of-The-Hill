let oneGameScreen = function($container, docId, leagueName){ 
    ...
    $container.html(
        "<br/><button id = 'oneGameScreenBackBtn' onclick = 'games($(\"#home\"), \""+leagueName+"\")'>back</button></p>"+
		"<div id = 'oneGameScreen'>"+
			...
			"<div id = 'guessWithOutcome'>guess With Outcome:</p>"+
				...
				"</div>"+
			"</div></p>"+
			"<div id = 'alternativeOutcome'>alternative Outcome (primum)<p/>"+
				...
				"</div>"+
			"</div>"+
			"<div id = 'alternativeOutcome2'>More Alternative Outcome (primum)<p/>"+
				"<div id = 'alternativeGuestScore'>"+
					"<input type = 'text' id = 'guestGuess' placeholder = 'Guest'/>"+
				"</div>-"+
				"<div id = 'alternativeHomeScore'>"+
    ...
    let docRef = firebase.firestore().collection("leagues/"+leagueName+"/games").doc(docId);//=====doc(docId.id)
    docRef.get().then(function(doc) {
        if (doc.exists) {
            addGame(doc, leagueName);	//add the game from DB to screen
			$("#oneGameScreen #goToGame").css("display", "none");	//hide goToGame button
        } else {
